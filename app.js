...
function arrayDiff(a, b) {
  ...
  for(let i = 0; i < b.length; i++){
    yourArray = a.filter((item, index) => item !== b[i] 
...
function largestOfFour(arr) {
  let largestNumarrays = [] ;
  let newArrs;
 let l = arr.length;
 for(let i = 0; i < l; i++){
 newArrs =  arr[i].sort(function(a,b){ return a - b})
 let m = newArrs.length
  largestNumarrays.push(newArrs[m - 1])
}
  return largestNumarrays;
}
...
function getIndexToIns(arr, num) {
  ...
  arr.push(num)
  arr.sort((function(a,b){return a - b}))
indexOfNew = arr.indexOf(num)

  console.log(indexOfNew)
  return indexOfNew;
}

getIndexToIns([20, 40, 10, 30, 50], 35);